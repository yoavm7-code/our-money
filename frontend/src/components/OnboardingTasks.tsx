...
export default function OnboardingTasks() {
  ...
  const TaskIcon = ({ name }: { name: string }) => {
    switch (name) {
      case 'bank':
        return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><circle cx="12" cy="12" r="2"/><path d="M6 12h.01M18 12h.01"/></svg>;
      case 'upload':
      ...
      case 'wallet':
        return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><path d="M16 14h.01"/><path d="M2 10h20"/></svg>;
      case 'target':
    ...
    <div className="card border-2 border-primary-200 dark:border-primary-800/50 animate-fadeIn">
      <div className="flex items-start justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{t('onboardingTasks.subtitle')}</p>
        </div>
        <button
          type="button"
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between text-xs mb-1.5">
          <span className="text-slate-500">{t('onboardingTasks.progress')}</span>
          <span className="font-medium text-primary-600 dark:text-primary-400">{completedCount}/{totalCount}</span>
        </div>
            ...
            <Link
              ...
              className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-200 group ${
                isDone
                  ...
                  : 'bg-slate-50 dark:bg-slate-800/50 hover:bg-primary-50 dark:hover:bg-primary-900/10'
              }`}
            >
              <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                isDone
                  ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'
                  : 'bg-slate-200 dark:bg-slate-700 text-slate-500 group-hover:bg-primary-100 dark:group-hover:bg-primary-900/30 group-hover:text-primary-600 dark:group-hover:text-primary-400'
              }`}>
                {isDone ? (
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${isDone ? 'line-through text-slate-400' : ''}`}>
                  {t(task.titleKey)}
                </p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t(task.descKey)}</p>
              </div>
              {!isDone && (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0 text-slate-300 group-hover:text-primary-500 transition-colors rtl:rotate-180">
